...
export function setupListeners() {
    ...
    const buildKLayoutPyaClipboardText = (polygons) => {
        // polygons: Array<Array<[x,y]>> in world/layout coords.
        const lines = [];
        // The magic header allows a KLayout macro to safely detect our payload.
        lines.push('# gds-preview: polygons v1');
        lines.push('# KLayout Python (pya) snippet generated by gds-preview');
        lines.push('# To paste as shapes, use the KLayout macro in klayout/paste_gds_preview_polygons.py');
        lines.push('import pya');
        lines.push('polys = [');
        for (const poly of polygons) {
            if (!Array.isArray(poly) || poly.length < 3) continue;
            const pts = poly
                .filter(p => Array.isArray(p) && p.length >= 2)
                .map(p => `pya.DPoint(${fmtNum(p[0])}, ${fmtNum(p[1])})`);
            if (pts.length < 3) continue;
            lines.push(`    pya.DPolygon([${pts.join(', ')}]),`);
        }
        lines.push(']');
        return lines.join('\n');
    };
    ...
    const copyHighlightedToClipboard = async () => {
        ...
        const text = buildKLayoutPyaClipboardText(polys);
        if (!text) {
            updateStatus('Nothing highlighted to copy');
            return;
        }
        ...
        try {
            if (navigator.clipboard && navigator.clipboard.writeText) {
                ...
                updateStatus(`Copied ${polys.length} polygon(s) to clipboard (KLayout/pya)`);
                return;
            }
        } catch (_) {
    ...
    const finalizeBoxSelect = (x0, y0, x1, y1) => {
        const sel = {
            minX: Math.min(x0, x1),
            maxX: Math.max(x0, x1),
            minY: Math.min(y0, y1),
        ...
        if (state.currentEngine === 'webgl' && state.config && state.config.engineType === 'rust') {
            ...
            state.boxSelectPending = { token: snapToken, sel };
